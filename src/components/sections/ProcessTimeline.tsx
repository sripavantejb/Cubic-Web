...
export function ProcessTimeline() {
  ...
  return (
    <section ref={root} className="section-x section-y bg-paper-2">
      <div className="mx-auto max-w-[1440px]">
        ...
        <div className="relative mt-10 md:mt-16">
          ...
          <ol className="grid gap-8 md:grid-cols-2 md:gap-10 lg:grid-cols-4">
            {process.steps.map((step) => (
              <li key={step.title} className="process-step">
                ...
                <h3 className="mt-3 text-[18px] font-medium md:text-[22px]">{step.title}</h3>
                <p className="mt-2 text-[15px] leading-relaxed text-muted md:mt-3 md:text-[16px]">{step.text}</p>
              </li>
            ))}
          </ol>
        </div>
