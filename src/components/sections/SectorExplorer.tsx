  ...
  return (
    <section id="sectors" className="bg-charcoal text-paper">
      <div className="section-x section-y mx-auto max-w-[1440px]">
        ...
        <div className="mt-10 grid gap-8 lg:mt-16 lg:grid-cols-[0.9fr_1.1fr]">
          <ul>
            {sectors.items.map((item, i) => (
              <li key={item.title}>
                <button
                  ...
                  className={cn(
                    "flex min-h-12 w-full items-baseline justify-between gap-3 border-t border-white/10 py-4 text-left md:gap-4 md:py-5",
                    i === active ? "text-paper" : "text-paper/40 hover:text-paper/75",
                  )}
                >
                  <span className="min-w-0 text-[16px] font-medium md:text-[22px]">{item.title}</span>
                  <span className="meta shrink-0 text-mist">{item.tag}</span>
                </button>
              </li>
            ))}
          </ul>
          <div className="relative aspect-[16/11] min-h-[14rem] overflow-hidden max-lg:order-first lg:aspect-auto lg:min-h-[420px]">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 sm:p-6 md:p-8">
                  <p className="text-[14px] leading-relaxed text-paper/90 md:text-[16px]">{current.text}</p>
