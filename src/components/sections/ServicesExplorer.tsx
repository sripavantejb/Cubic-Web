...
export function ServicesExplorer() {
  ...
  return (
    <SectionFrame id="services" className="bg-charcoal text-paper">
      <SectionHeading light eyebrow={services.eyebrow} heading={services.heading} lede={services.lede} />

      <div className="mt-5 hidden min-h-0 flex-1 gap-10 lg:grid lg:grid-cols-[0.9fr_1.1fr]">
        <ul className="flex min-h-0 flex-col justify-center">
          {services.items.map((item, i) => (
            <li key={item.title}>
              <button
                ...
                className={cn(
                  "flex w-full items-center justify-between border-t border-white/10 py-3 text-left transition-colors",
                  i === active ? "text-paper" : "text-paper/45 hover:text-paper/80",
                )}
              >
                <span className="meta">{item.num}</span>
                <span className="display text-[clamp(1.2rem,2vw,1.85rem)]">{item.title}</span>
              </button>
            </li>
          ))}
        </ul>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="absolute inset-x-0 bottom-0 p-6">
                <Icon className="mb-3 size-5 text-mist" />
                <p className="max-w-md text-[14px] leading-relaxed text-paper/90">{current.text}</p>
                <div className="mt-4 flex gap-2">
                  {gallery.map((src, i) => (
                    <button
                      ...
                      onClick={() => setShot(i)}
                      className={cn(
                        "relative h-11 w-16 overflow-hidden border",
      ...
      <div className="mt-6 min-h-0 flex-1 overflow-y-auto divide-y divide-white/10 lg:hidden">
        {services.items.map((item, i) => {
          ...
          return (
            <div key={item.title}>
              <button
                ...
                onClick={() => setActive(i)}
              >
                <span className="display text-[1.35rem]">{item.title}</span>
              ...
              <AnimatePresence>
                {open ? (
                  ...
                  >
                    ...
                    <p className="pb-5 text-[15px] text-paper/80">
                      <ItemIcon className="mb-2 size-5 text-mist" />
                      {item.text}
                    </p>
                  </motion.div>
                ) : null}
