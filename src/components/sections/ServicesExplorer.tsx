  ...
  return (
    <section id="services" className="bg-charcoal text-paper">
      <div className="section-x section-y mx-auto max-w-[1440px]">
        ...
        <div className="mt-10 hidden gap-12 lg:mt-16 lg:grid lg:grid-cols-[0.9fr_1.1fr]">
          <ul>
          ...
          {services.items.map((item, i) => {
            ...
            return (
              <div key={item.title}>
                <button
                  type="button"
                  className="flex min-h-12 w-full items-start justify-between gap-4 py-4 text-left sm:items-center sm:py-5"
                  aria-expanded={open}
                  onClick={() => setActive(i)}
                >
                  <span className="display min-w-0 flex-1 pr-3 text-[clamp(1.25rem,5.5vw,1.5rem)] leading-tight">{item.title}</span>
                  <span className="meta shrink-0 pt-1 text-mist sm:pt-0">{item.num}</span>
                </button>
                <AnimatePresence>
                  {open ? (
                    <motion.div
